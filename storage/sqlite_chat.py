...
def save_message(
    ...
    with _get_connection() as conn:
        conn.execute(
            ...
            INSERT INTO messages
                (message_id, conversation_id, session_id, role,
                 message_text, created_at, document_scope, grok_request_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (message_id, conversation_id, session_id, role,
             message_text, created_at, document_scope, grok_request_id),
    ...
    logger.info("Saved message | session_id=%s | role=%s | message_id=%s", session_id, role, message_id)
    return message_id
...
def _enforce_session_limit(conn: sqlite3.Connection, session_id: str) -> None:
    ...
    if count > _MAX_MESSAGES_PER_SESSION:
        ...
        logger.warning("Session message limit reached; pruned %d old messages | session_id=%s", excess, session_id)
