...
from storage.sqlite_chat import (
...
def render_chat_ui() -> None:
    ...
    st.caption(
        f"Session: {session_id}  •  Documents: {', '.join(completed)}"
    )
    ...
    # Chat input must be declared outside the container
    ...
    with messages_container:
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner("Searching documents…"):
                ...
                collections, needs_text, needs_image = route_query(
                    user_query
                )

                logger.info(
                    "Chat query | session_id=%s | query_preview=%.60s",
                    session_id,
                    user_query,
                )
                ...
                if not raw_results:
                    ...
                    answer = (
                        "No documents have been indexed in this session yet, "
            ...
            with st.spinner("Generating answer…"):
                answer, grok_request_id = ask_grok(
                    user_query,
                    context,
                )
            ...
            decision = st.radio(
                ...
                horizontal=True,
                key=f"decision_{conversation_id}",
            )

            review_note = st.text_area(
                "Reviewer Notes (Optional)",
                key=f"review_note_{conversation_id}",
            )

            if st.button(
                "Save Review",
                key=f"save_review_{conversation_id}",
            ):
                save_review(
            ...
            citation_md = format_citations_markdown(
                citations
            )
            ...
            logger.info(
                "Chat turn complete | session_id=%s | grok_request_id=%s",
                session_id,
                grok_request_id,
            )